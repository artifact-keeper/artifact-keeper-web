...
- **Package Age Policy now shows the saved policy instead of hardcoded defaults** (#658) - the repo Settings "Package Age Policy" form (feature #265) seeded from local defaults (disabled / 3 days) and never read the persisted config, so a saved policy always looked unset on revisit — even though the PATCH did persist. `adaptRepository` now maps the `quarantine_enabled` / `quarantine_duration_minutes` fields the backend already returns on read (backend #1841), and the form seeds the toggle/value/unit from them via a `dirty`-gated sync (adds a `minutesToAge` inverse of `ageToMinutes`). A disabled policy still keeps its remembered duration, matching the backend which gates purely on the enabled flag.
