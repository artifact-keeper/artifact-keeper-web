...
### Added
- **Three-state repository visibility control** (#839, backend migration 212) - the binary "Public repository" switch is replaced by a **Visibility** select offering `Public`, `Internal` and `Private`, shared verbatim by the create dialog, the edit dialog and the repository Settings tab (`visibility-select.tsx`, the same shared-component pattern as `format-config-fields.tsx`) so the three surfaces cannot drift. `Internal` is the state the switch could not express: readable by every signed-in user with no grant, never readable anonymously. Each option carries a one-line hint describing who can actually read the repository. The control writes the backend's new `visibility` field rather than the deprecated `is_public` boolean, and falls back to deriving the state from `is_public` when reading a backend that predates the field, so the UI is safe to deploy ahead of the server.

  The repository list row now distinguishes the three states as well. A padlock previously meant "not public", which would have rendered `internal` identically to `private` — the single most consequential difference a list row can show. `Public` is unmarked as before, `Internal` shows a group icon, `Private` keeps the padlock, and both carry an `aria-label` spelling out who can read.
...
### Changed
- **When guest access is disabled the visibility control stays visible; only the `Public` option is withdrawn.** The switch used to be hidden entirely and replaced with "Public repositories are disabled by the operator", which made sense when there were two states and one was unavailable. With three, the choice between `Internal` and `Private` is still meaningful and still the operator's to make, so the control remains and the note explains that internal repositories are still readable by every signed-in user. A repository already marked `Public` from before the policy changed keeps rendering its own value rather than an empty control, so opening its settings cannot silently move it on save.
