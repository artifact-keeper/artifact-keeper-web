...
### Added
- **Silent SSO auto-login (check-sso)** (#812, backend artifact-keeper#3546) - users who already hold a live IdP session (e.g. Keycloak) previously had to click **Sign in with …** on every fresh visit; now the app signs them in invisibly. On load, when nobody is signed in, an enabled OIDC provider exists, and no prior attempt is recorded for this browser session, the app probes the IdP once with `prompt=none` inside a **hidden same-origin iframe** — never a top-level redirect. A live IdP session completes the normal callback invisibly (cookies set, identity adopted, auth-scoped queries refetched); no session means the backend relays the IdP's `login_required` as `/callback?silent_denied=1` and the visitor stays anonymous with **no error UI** — anonymous registry browsing remains first-class, which is what separates this from a naive sole-provider auto-redirect. **Fail open:** an unreachable or slow IdP just times the probe out (5s) and browsing continues anonymously; it can never strand a visitor on a browser error page or an uninvited Keycloak login screen. Exactly one attempt per browser session (sessionStorage guard, written before the probe starts so a crash cannot loop), cleared on explicit sign-in and sign-out — so signing out of Artifact Keeper while the IdP session lives on means the next visit signs back in, the standard check-sso semantic. The explicit "Sign in with …" button, `?fallback=local`, LDAP, and local login are unchanged. **Operator kill switch:** backend `OIDC_SILENT_SSO=false` (surfaced as `auth.silent_sso_enabled` in `GET /api/v1/system/config`; defaults on, including against older backends that omit the field — there the probe fails invisibly once and stops). **Trade-off accepted:** the SSO callback routes (`/callback`, `/auth/callback`) relax `frame-ancestors 'none'`/`X-Frame-Options: DENY` to same-origin only (`'self'`/`SAMEORIGIN`) so the probe iframe can land there; same-origin framing is not a clickjacking vector and every other route stays unframable. No probe starts on `/login`, `/callback`, or `/change-password`, nor during first-boot setup or demo mode auto-login.
