...
### Added
- **SSO admin UI parity for backend opt-ins shipped without a frontend** (#521, #522) - backfills the two per-provider toggles the backend added but the admin UI never surfaced (the "lockstep debt"). SAML provider form gains a **Use absolute ACS URL** switch (backend `use_absolute_acs_url`, migration 139) in the Sign Requests / Require Signed Assertions group, for stricter SAML 2.0 IdPs that reject relative AssertionConsumerServiceURLs. OIDC provider form gains an **Allow legacy RSA keys (< 2048-bit)** switch (backend `allow_legacy_rsa_keys`) beside Auto Create Users, with an amber "Below the OWASP ASVS 4.0 baseline — legacy/insecure, only enable if required" caveat kept at the point of decision (Lark AnyCross's 1024-bit RS256 key is the named use case). Both are off by default, sourced from the loaded config, and echoed on create + update. `src/types/sso.ts` and the `adaptOidcConfig` / `adaptSamlConfig` adapters propagate the new fields with a defensive `false` default so the UI is safe to deploy against a backend that predates the columns.
