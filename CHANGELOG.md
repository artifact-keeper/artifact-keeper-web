...
- **Per-artifact Security tab now surfaces in-tree scanner results** (#368) - the artifact's Security tab queried only SBOM CVE history and Dependency-Track, leaving rows from `scan_results` / `scan_findings` (image_scanner, package_scanner output) invisible on the artifact's own page. Adds an "Image / Package Scans" section between the CVE table and the Dependency-Track section, sourced from `securityApi.listArtifactScans(artifact.id)`. Each row shows scan type, status, finding counts, severity sub-counts, and links admins through to the existing `/security/scans/[id]` detail view (non-admins see "Admin only" since the detail route lives under the `(admin)` group). Includes loading skeleton, error banner, and uses the SDK's `total` field so the badge stays accurate when results are paginated.
