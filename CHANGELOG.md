...
### Fixed
- **A long artifact filename no longer pushes the artifact table off the repository detail panel** (#768) - a single artifact with a long name widened the flat artifact table past the panel's right edge, and the overflow was clipped rather than scrollable: Size, Downloads, Created and the per-row Details/Download actions were unreachable, with no horizontal scrollbar and no trackpad, wheel or keyboard path to them. Name was the only wide column with no width constraint (Path beside it was already capped with `max-w-[200px] truncate`), and since both table primitives force `whitespace-nowrap`, nothing wrapped and the column took the full rendered width of the longest filename on the page. That width then escaped the table: the panel's `ScrollArea` viewport lays children out in a `display: table` box (shrink-to-fit above a 100% floor) and is permanently `overflow-x: hidden` while only a vertical scrollbar mounts, so the excess applied to the entire detail column and displaced right-aligned controls in unrelated cards above it, such as the Storage card's Estimate button. The Name cell is now capped at `max-w-[360px]` and elided through a new `MiddleEllipsis` component that pins the last 12 characters, because artifact names are distinguished by their tail as often as their head (`...-tlsconsul` vs `...-tlsconsul-docker`, plus version suffixes, variant suffixes and file extensions) and end-truncation would have rendered such rows visually identical. The complete name stays in the DOM, so it remains selectable, copyable and announced in full by screen readers, and it is also reachable via the cell's native tooltip and the artifact detail dialog. **Still open:** the app-wide half of the fix (mounting `<ScrollBar orientation="horizontal" />` so horizontal overflow anywhere becomes reachable) was dropped on maintainer preference against introducing a horizontal scrollbar, so every other `ScrollArea` call site still clips horizontal overflow silently.
- **Repository artifact pagination reports a real total instead of one that grows with the page number** (#767) - the bar under a repository's artifact list read `1-20 of 21` on page 1, `21-40 of 41` on page 2 and `41-60 of 61` on page 3, with the page count stuck one ahead of the current page, regardless of how many artifacts the repository actually held. The arithmetic in `DataTablePagination` was sound; the value handed to it was not a total. The keyset-paged listings (backend artifact-keeper#2520 / #2519 / #2518) deliberately avoid a whole-catalog `COUNT` on every page request, so `pagination.total` falls back to a cheap lower bound (`offset + returned + has_more`) with `has_more` as the authoritative next-page signal, and exact counts are opt-in through `?count=exact`, which the web client never sent. Since the bar derives both the range label and `totalPages` from `total`, that one lower-bound value produced both reported symptoms. `ListArtifactsParams` now carries `count`, `buildArtifactsListPath` forwards it (`listGrouped` builds its query string by hand, so the new param had to be threaded through explicitly), and the repository content query asks for `count: "exact"`. No backend change was needed, and because one query feeds the flat table, the Maven component list and the Docker tag list, all three views are corrected together. The cost is one extra `COUNT` per page load, accepted on this surface because it renders both a range and a page count and so genuinely needs a real total; listings that only need "is there a next page" should leave `count` unset and read `has_more`.
