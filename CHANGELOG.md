...
### Security
- **Pin third-party GitHub Actions to commit SHAs** (#205) - all `uses:` lines in `codeql.yml`, `dependency-review.yml`, `docker-publish.yml`, and `stale.yml` are now pinned to specific commit SHAs (with version comments) so an upstream tag rewrite cannot silently swap action code. `ci.yml` was already pinned and is the model. Dependabot is configured for `github-actions`, so bumps continue to flow through review.
