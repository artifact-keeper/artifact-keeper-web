...
- **Every page now ships in Simplified Chinese (in addition to English)** (user-requested i18n follow-up, no linked GitHub issue — bd/issue tooling unavailable for this change) - the entire web UI is translated into `zh`: the admin console (approvals, migration, downloads, license policies, groups, users, lifecycle, monitoring, permissions, promotion rules, quality checks/gates, rate limits, security incl. blast radius + DT projects/policies/scans, curation, analytics, audit, backups, sync policies, signing, system health, telemetry, settings incl. SSO, service accounts, error/age-gate), the protected pages (access tokens, peers, plugins, profile, replication, webhooks), and the app pages (dashboard, repositories with every detail/dialogs/settings/storage/security/SBOM/labels/versions tabs, packages, staging incl. promotion/rejection/history, search, builds, change-password) plus all shared components (data tables + pagination, file browser/viewer/upload, confirm dialogs, toasts, quarantine banners/badges, quick search, trend charts, severity/risk gauges, repo setup guide, npm upstream card). Untranslated strings still fall back to English, so no key ever leaks to the UI. Existing tests were updated where they asserted rendered copy, and a global next-intl test mock (`vitest.next-intl.setup.ts`) makes translated components testable without per-file mocks. A complete en/zh key parity check plus a successful production build are part of the change.
- **UI is now localizable — an English/Chinese language switch is shipped** (user-requested i18n work, no linked GitHub issue — bd/issue tooling unavailable for this change) - the web UI gains a localization framework (next-intl) so its text can be translated without changing any route path: the locale is resolved per-request from a `NEXT_LOCALE` cookie rather than a URL prefix, which keeps existing deep links, bookmarks and the Playwright E2E specs working unchanged. A language switcher appears in the app header and on the login page, and the app shell (sidebar groups + all navigation items, header, demo and password-expiry banners, skip-nav link, instance switcher) plus the entire login flow (form, TOTP, first-time setup, account-lockout, SSO button labels) are fully translated into Simplified Chinese (`zh`). Remaining pages are migrated incrementally in follow-up changes; any not-yet-translated string falls back to the English message, so nothing regresses to a raw key while the migration is in flight. Adding a new language later is a one-file change (`messages/{locale}.json`) plus registering the code in `src/i18n/routing.ts`.
...
- **Admin message namespaces are now nested under a single `admin` key** (user-requested i18n follow-up, no linked GitHub issue — bd/issue tooling unavailable for this change) - the 35 flat `admin*` namespaces in the admin message group are collapsed into one top-level `admin` object (`adminUsers` → `admin.users`, `adminSettingsSso` → `admin.settingsSso`, …), so the redundant `admin` prefix no longer repeats across the catalog. Components reference the nested path via `useTranslations("admin.users")` (next-intl supports dotted namespaces); all 54 call sites across the 35 admin pages/error boundary were updated, and the global vitest next-intl mock now resolves dotted namespace paths. Rendered output is byte-identical — this is purely structural. The other `repo*` / `artifact*` / `package*` prefixes are kept because they disambiguate globally (e.g. `adminScans` vs `artifactScans`).
- **Message catalogs are now split by functional domain and loaded per route** (user-requested i18n follow-up, no linked GitHub issue — bd/issue tooling unavailable for this change) - the monolithic `src/i18n/locales/{en,zh}.json` catalogs (189 KB / 182 KB) are split into 12 per-feature group files under `src/i18n/locales/{en,zh}/` (core, auth, app, guides, protected, admin, repositories, packages, builds, search, setup, staging) that mirror the app's route structure. `request.ts` now serves only the always-on `core` namespaces globally, and each route provides its own groups through a nested `NextIntlClientProvider` (auth, app shell, protected, admin, plus new per-feature layouts for repositories / packages / builds / search / setup / staging). A route only ships the messages its subtree renders, cutting the per-page client payload: e.g. admin pages no longer receive the repositories / staging / … namespaces, and the login page no longer receives the admin catalog. Rendered output and message content are byte-identical to before; a `scripts/check-i18n.mjs` script now validates group coverage and en/zh deep-key parity.
...
- **Signing-key revoke toast and the service-accounts page description showed the wrong text** (user-requested i18n follow-up, no linked GitHub issue — bd/issue tooling unavailable for this change) - two message catalogs (`adminSigning`, `adminServiceAccounts`) each contained the same key twice with different meanings: `adminSigning.revoked` was both the revoke-success toast ("Signing key revoked") and the revoked status badge label ("revoked"), and `adminServiceAccounts.description` was both the page intro and the create/edit form's "Description" field label. JSON parsers silently keep the last duplicate, so under both locales the toast rendered the short status label and the page intro rendered the bare field label. The status badge now uses `revokedLabel` and the form labels use `descriptionLabel`, restoring the full texts; a duplicate-key check now passes for both catalogs.
- **Closing the CI/CD platform dialog on the Setup page crashed with `MISSING_MESSAGE: Could not resolve \`setup\``** (user-requested i18n follow-up, no linked GitHub issue — bd/issue tooling unavailable for this change) - the integration dialog derived its title from `t(selectedPlatform?.nameKey ?? "")`, and Radix's controlled dialog keeps the content mounted for the close animation after `onOpenChange(false)` has already cleared the selected platform. During that one frame `selectedPlatform` is `null`, so the header called `t("")` and next-intl tried to resolve the empty key — reporting the `setup` namespace as unresolvable. The crash was only visible in a real browser (jsdom has no exit animation, so the unit tests never exercised the frame) and only under a locale whose message file had the namespace, which is why it surfaced after the zh localization landed. The dialog header is now guarded by `{selectedPlatform && …}`, and the close/reopen test now re-opens the dialog and asserts the title still renders.
- **A few pages and base components still rendered English under the Chinese locale** (user-requested i18n follow-up, no linked GitHub issue — bd/issue tooling unavailable for this change) - the first localization pass covered the shell, admin console, protected pages and app pages but missed the package detail page, the setup guide page, the SSO callback / auth-error / 403 / 500 error pages, and several shadcn base components (dialog/sheet close label, sidebar mobile header + toggle, command palette, breadcrumb ellipsis). Under `zh` those surfaces silently fell back to English — for entirely untranslated pages because they never called `useTranslations`, and for shared components because their accessibility labels were hardcoded. The package detail page (breadcrumbs, tabs, metadata grid, versions table, install block) and setup guide page (header, search, category filters, empty states, repo count plural, CI/CD platform cards + their setup steps, both dialogs) are now fully translated, the error pages gained `errorPages` / `authError` / `ssoCallback` namespaces, and the base components translate their sr-only/aria labels via a new `ui` namespace. Also completed two earlier translation gaps: the `setupGuide` variant labels `SBT`/`Npm`/`Bun`/`Pip`/`Uv` were never in the `SETUP_TEXT_KEYS` map, and the SSO providers, peers status filter and builds info labels referenced keys (`ldapProviders`, `statusOnline`, `infoModules`, …) that did not exist in the message files — all now resolve. en/zh key parity is verified recursively.
