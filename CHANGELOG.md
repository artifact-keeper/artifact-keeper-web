...
- **Setup Guide had no vscode onboarding for the Open VSX gallery gateway; create-repository had no default upstream URL for it** (#793, backend artifact-keeper/artifact-keeper#3253) - `vscode` had no case in `getRepoSetupSteps`, so a vscode repository's Setup Guide fell through to the generic `curl -X PUT .../artifacts/` upload snippet, which is not how VSCodium, code-server, or VS Code talk to a gallery. The backend's own client-config doc was deleted from artifact-keeper/artifact-keeper#3253 before merge, so the shipped 1.8.0 gateway had no user-facing onboarding anywhere. The Setup Guide now covers all three supported clients: VSCodium's persistent `product.json` `extensionsGallery` block (`serviceUrl`/`itemUrl`/`extensionUrlTemplate`/`latestUrlTemplate`, with a callout that `latestUrlTemplate` is required — omitting it can let an update lookup fall back to open-vsx.org directly, bypassing the gateway), code-server's `EXTENSIONS_GALLERY` env block, and official VS Code's enterprise-policy constraint (desktop only, requires the `ExtensionGalleryServiceUrl` policy + an entitled account + a supported policy deployment — not a general preference toggle). The create-repository dialog now suggests `https://open-vsx.org/vscode/gallery` as the default upstream URL for a new vscode Remote, matching the backend's `/vscode/gallery`-suffix validation (`backend/src/api/handlers/vscode.rs`).
