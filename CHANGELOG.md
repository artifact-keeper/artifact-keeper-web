...
### Fixed
- **Granting a service account repository access was impossible from the Permissions page: it was listed as a User, where the grant always failed** (#823, backend artifact-keeper#3634) - service accounts are rows in the `users` table, so `GET /api/v1/users` returns them alongside people, and the admin **Permissions** create form offered them in the principal picker under its default principal type, **User**. Submitting that grant could never work: the API accepts a service-account row only as `principal_type: "service_account"` and rejects it as a user with `400 principal_id <uuid> does not exist as a user`, a message that names no remedy and appears after the operator has already filled in the target and actions. The failure lands at the worst moment -- a CI service account gets `403 You do not have permission to perform this action on this repository` on its first upload (token scopes only narrow, they never grant), and the obvious fix, granting it `write` on the repository, is the very thing this picker blocked. The **User** list now excludes accounts that appear in the service-account list, so the only place a service account can be picked is the **Service Account** type, where the grant succeeds. Selecting the principal type first was already the workaround; it is no longer needed. **Trade-offs:** the exclusion is computed client-side by subtracting the ids of the separately-loaded service-account list, because `GET /api/v1/users` exposes no `is_service_account` field and no filter to ask the server for people only -- artifact-keeper#3634 adds both, after which this becomes a server-side `is_service_account=false` query and stops depending on the page happening to load every user in one page. Until then the picker waits for the service-account list before enabling ("Loading principals..."), since an unfiltered User list rendered for even one frame puts the rejected principal back on screen. The other four admin screens reading the same endpoint -- the users list, the group member picker, the audit actor filter and the age-gate approver picker -- still show service accounts as ordinary users and are unfixable here for the same reason; they are covered by artifact-keeper#3634.
