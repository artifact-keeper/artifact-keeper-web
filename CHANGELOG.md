...
### Added
- **Silent SSO auto-login (check-sso)** (#812, backend artifact-keeper#3546) - users who already hold a live IdP session (e.g. Keycloak) previously had to click **Sign in with …** on every fresh visit; now the app signs them in invisibly. On load, when nobody is signed in, an enabled OIDC provider exists, and no prior attempt is recorded for this browser session, the app makes one silent `prompt=none` round trip through the IdP: a live IdP session completes the normal callback invisibly and the user lands back on the page they were on, signed in; no session means the IdP answers `login_required`, the backend relays it as `/callback?silent_denied=1`, and the visitor lands back where they were, still anonymous, with **no error UI** and **never a visible IdP login page** — anonymous registry browsing remains first-class, which is what separates this from a naive sole-provider auto-redirect. **Fail open:** before navigating anywhere, a short-timeout (4s) same-origin preflight fetches the `prompt=none` login URL with redirects disabled; the backend only answers with a redirect after its own server-side fetch of the IdP discovery document succeeded, so a down, unreachable, or hung IdP fails the preflight and nothing happens — a visitor can never be stranded on a browser error page. **Trade-off accepted:** the probe is a top-level navigation, not a hidden iframe, deliberately: IdP session cookies are frequently `SameSite=Lax` (Keycloak issues Lax unless certain the deployment is HTTPS-proxied), and Lax cookies are never sent on a cross-site iframe navigation — an iframe probe silently reports "no session" for users who have one; the top-level form works on both Lax and None IdPs at the cost of one invisible page round trip per session. Exactly one attempt per browser session (sessionStorage guard, written before the redirect so an interrupted round trip cannot loop), cleared on explicit sign-in and sign-out — so signing out of Artifact Keeper while the IdP session lives on means the next visit signs back in, the standard check-sso semantic. The explicit "Sign in with …" button, `?fallback=local`, LDAP, and local login are unchanged; no probe starts on `/login`, `/callback`, or `/change-password`, nor during first-boot setup or demo mode auto-login. **Operator kill switch:** backend `OIDC_SILENT_SSO=false` (surfaced as `auth.silent_sso_enabled` in `GET /api/v1/system/config`; defaults on, including against older backends that omit the field — there the probe's preflight fails invisibly once and stops).
