...
### Added
- **JupyterLab Extensions in the repository format dropdown + Extension Manager set-me-up** (#833, backend artifact-keeper#3784 implemented in artifact-keeper#3786 and artifact-keeper#3783 implemented in artifact-keeper#3788) - users hosting JupyterLab extensions had to know that prebuilt extensions are pip wheels and create a plain PyPI repository, and nothing told them how to point JupyterLab 4's Extension Manager sidebar at it. The create dialog now offers **JupyterLab Extensions** (`jupyter`), filed in the Extensions group for the repository-list filter; a `jupyter` repository is treated exactly like `poetry` — a PyPI alias — so it gets the Packages default tab, `pip install` snippets and the PyPI routes. The Setup tab / set-me-up dialog gains a **JupyterLab** client tab (the default for `jupyter` repos, a secondary tab on plain `pypi` repos) with the pip index config for the install side, `pip install --index-url <base>/pypi/<repo>/simple/ <extension>`, the `jupyter_lab_config.py` line `c.PyPIExtensionManager.base_url = "<base>/pypi/<repo>/pypi"`, and a note that source extensions come from an npm repository and nbextensions via pip or conda. The Extension Manager line is shown only for hosted and virtual repositories (virtual adds that members must be hosted); on a remote repository the backend faults XML-RPC `browse`, so the tab shows a note to use a hosted or virtual repository instead. **Trade-offs:** `PyPIExtensionManager` has no credential setting, so the sidebar catalogue only works on a repository that allows anonymous read — the tab says so rather than hiding it. The local `RepositoryFormat` union is hand-maintained (the published SDK types `format` as a plain `string`), so a backend older than artifact-keeper#3786 still rejects creating a `jupyter` repository even though the dropdown offers it.
...
### Security
- **Clear the `npm audit --audit-level=high` advisories that turned the `Dependency Audit` CI job red on every fresh PR run** (#836) - `main`'s lockfile had aged into 10 advisories (1 critical, 5 high, 4 moderate), so any PR whose CI ran after the advisories were published failed the gate for reasons unrelated to its diff. `next` 16.2.12 → 16.3.3 (GHSA-p293-qw3h-jr36 and GHSA-2xp9-vwfh-vxw4, both critical unauthenticated RCE — 16.3.3 is the smallest release carrying the fixes; `next` is pinned exactly, so this is a deliberate `package.json` bump rather than `npm audit fix --force`), `sharp` 0.35.3 → 0.35.4 via the existing `overrides.next.sharp` floor (GHSA-rgj7-g3m4-5g8c, libheif), `js-yaml` 4.3.1 → 4.3.2 via the existing `overrides.js-yaml` floor (GHSA-2883-xcg3-v3hh, transitive through `@hey-api/openapi-ts`), `vitest` / `@vitest/coverage-v8` / `@vitest/mocker` 4.1.10 → 4.1.11 (GHSA-82fw-gwwq-j7x9), and `@humanfs/node` 0.16.7 → 0.16.8 (GHSA-p498-v437-472g, transitive through `eslint`). Audit is now clean at every level. **Trade-off / trap noted:** Next 16.3.0 turned on `experimental.useTypeScriptCli` by default, so `next build` now type-checks the whole `tsconfig.json` `include` set (test files included) instead of only app code; 22 pre-existing `tsc --noEmit` errors in `audit/__tests__/page.test.tsx` and `security/blast-radius/__tests__/page.test.tsx` (mock helpers defaulting `data: undefined`, which TypeScript narrowed to the literal `undefined` type) would have failed the build, so both helpers now declare `data: undefined as unknown`. `tsc --noEmit` is at 0 errors as a result. Also, `npm audit fix` and `npm install vitest@4.1.11` crash npm 10.9.2's arborist (`Cannot read properties of null (reading 'edgesOut')` while walking vitest's optional-peer chain) — the lockfile was regenerated with npm 11.19.1 and then verified with a clean `npm ci` on npm 10.9.2 / Node 22, which is what CI runs.
