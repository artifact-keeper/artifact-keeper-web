...
### Added
- **JupyterLab Extensions in the repository format dropdown + Extension Manager set-me-up** (#833, backend artifact-keeper#3784 / artifact-keeper#3783) - users hosting JupyterLab extensions had to know that prebuilt extensions are pip wheels and create a plain PyPI repository, and nothing told them how to point JupyterLab 4's Extension Manager sidebar at it. The create dialog now lists **JupyterLab Extensions** (`jupyter`) in the Extensions group next to VS Code and JetBrains; a `jupyter` repository is treated exactly like `poetry` — a PyPI alias — so it gets the Packages default tab, `pip install` snippets and the PyPI routes. The Setup tab / set-me-up dialog gains a **JupyterLab** client tab (the default for `jupyter` repos, a secondary tab on plain `pypi` repos) with `pip install --index-url <base>/pypi/<repo>/simple/ <extension>`, the `jupyter_lab_config.py` line `c.PyPIExtensionManager.base_url = "<base>/pypi/<repo>/pypi"`, and a note that source extensions come from an npm repository and nbextensions via pip or conda. **Trade-off:** the local `RepositoryFormat` union is hand-maintained (the published SDK types `format` as a plain `string`), so this ships ahead of the backend enum; until artifact-keeper#3784 lands the backend rejects creating a `jupyter` repository, and until artifact-keeper#3783 lands the Extension Manager endpoints under `/pypi/<repo>/pypi` return 404 even though the snippet is shown.
