...
### Added
- **`ApiError` on `apiFetch`** (#651) - `src/lib/api/fetch.ts` now throws an exported `ApiError` carrying `.status` and the raw `.body` instead of a bare `Error`, so callers can branch on the status without regex-matching the message. Additive and backward compatible: it extends `Error` and produces the byte-identical `API error <status>: <body>` message that existing consumers (notably `versions.ts`, which normalizes a 404 to an empty history list) already match on. Other modules are deliberately left on the message-matching path; migrating them is separate cleanup.
- **Age gate review queue: change a decision after the fact** (#651, backend artifact-keeper#2939) - a review's status is now a dropdown on each row rather than a pair of one-way approve/reject buttons, so an admin who approved the wrong package can put it back. Moving a decided review to `pending` calls the new `POST /api/v1/admin/age-gate/reviews/{id}/reopen`, which re-applies the gate and withholds the version again. Moving it straight to the other decision is reopen-then-decide, two calls presented as one action: if the second call fails the review really is left `pending`, and the page says so and refetches instead of reporting a plain failure or showing the status the operator started from. Reopen requires a non-blank reason (validated client-side before the call, since the backend 400s on a blank one); approve and reject still take an optional one. Approving now shows an explicit confirmation naming the version being released, and the dialog spells out which transitions run in two steps. The queue also surfaces the decision metadata it was already receiving: the recorded reason, the deciding admin (the response carries a user id, resolved to a name via the admin user list, falling back to a shortened id) and the decision date. Status filtering moved from a single-status dropdown to checkboxes that send the backend's comma-separated multi-status filter, with loading, empty, and error states for each. `reopen` is not in the generated SDK yet, so `src/lib/api/age-gate.ts` uses the shared `apiFetch` wrapper (same pattern as routing-rules / audit / downloads). Against a backend that predates artifact-keeper#2939 the endpoint is simply absent, and the page detects that as a capability rather than checking a version: the first reopen attempt that comes back 404 latches the capability off for the session, the transitions that need reopen become visibly unavailable with a one-line explanation, and deciding a still-pending review keeps working. A missing endpoint is told apart from a missing review id by the response body, since the handler's own 404 carries the backend's `{"code":"NOT_FOUND"}` envelope and an unrouted request does not, so a bad review id never disables the capability.
- **Quarantine release and reject from the artifact detail dialog** (#650, backend artifact-keeper#2940) - admins can now lift or finalize a quarantine hold from the repository artifact browser instead of calling the API by hand. The detail dialog gains **Release** and **Reject** actions (reject takes an optional reason), presented the same way the age gate review queue presents its approve/reject actions, and both are hidden for non-admins on top of the backend's `require_admin` gate. A rejection is terminal, so neither action is offered on an already rejected artifact, where the backend would only answer 409. The `QuarantineBanner` shipped by #253 is finally reachable: the artifacts listing now carries `is_blocked`, `quarantine_status`, `quarantine_until` and `quarantine_reason` per row, and an artifact opened through a surface that does not load them (the by-path detail endpoint behind the Maven grouped view) resolves its verdict through `GET /api/v1/quarantine/{artifact_id}` rather than being assumed clean. Those four keys are **absent**, not null, wherever the server did not look, and `@/lib/quarantine` keeps "not loaded" distinct from "loaded, and downloadable" so the banner cannot go dead again. The download control on a blocked artifact is disabled and relabelled with the reason instead of clicking through to a 409, and a successful action invalidates the artifacts listing and the artifact's quarantine status so the row reflects the new state without a manual refresh. The reason is redacted by the backend for callers who cannot access the repository; the banner and detail rows render correctly without it. The quarantine routes are not in the generated SDK yet, so `src/lib/api/quarantine.ts` uses the shared `apiFetch` wrapper (same pattern as `setReleaseTarget`). Note the field is `is_blocked`, not the `is_quarantined` the web previously read, which the backend never emitted. A queue view listing everything currently quarantined stays out of scope: the backend has no list endpoint, only per-artifact routes.
...
### Fixed
- **Release target now shows the saved link instead of always "None"** (#658) - the staging-repo Settings "Release Target" picker (feature #260) assumed the backend had no read endpoint and always seeded to "No release target", so a saved link never reappeared on revisit and the form looked like it hadn't saved — even though the PATCH did persist. Adds `repositoriesApi.getReleaseTarget` (`GET /api/v1/promotion/repositories/{key}/release-target`, the dedicated promotion endpoint the plain repository GET doesn't surface) and seeds the picker from it via a render-time sync gated on `dirty`, so a saved target is shown and a pristine form can't unlink an existing one on a stray click. The save path (PATCH) is unchanged.
...
- **Promote dialog honors a staging repo's linked release target** (#658) - the staging Promote dialog listed every release repo of the format and made the operator pick one every time, ignoring the configured link entirely — so the backend's "default to the link / reject a mismatched target" behavior never helped, and picking the wrong repo just produced a rejection error. It now reads the link via `getReleaseTarget` and, when one is set, locks the target to the linked repo (a read-only field with a "Linked release target" badge and a one-line explanation) instead of a free picker; unlinked staging repos keep the full picker unchanged.
