...
export function ProxySeverityBadges({
  ...
  /** Null when the backend suppressed the counts — render nothing, not "0". */
  findingsCount: number | null;
}) {
  if (findingsCount == null || findingsCount <= 0) return null;
  return (
...
/**
 * The CVEs behind a `vulnerable` verdict (artifact-keeper#3395).
 *
 * An empty recorded list is NOT rendered as a table with no rows: content
 * cached before per-CVE detail shipped legitimately has none, and a zero-row
 * findings table would read as "we looked and found nothing" — the same
 * false-clean this panel exists to remove.
 */
export function ProxyFindingDetail({
  detail,
}: {
  detail: ReturnType<typeof proxyFindingDetail>;
}) {
  if (detail.kind === "absent") return null;
  if (detail.kind === "none-recorded") {
    return (
      <p
        className="text-xs text-muted-foreground"
        data-testid="proxy-scan-no-cve-detail"
      >
        {PROXY_SCAN_NO_CVE_DETAIL_COPY}
      </p>
    );
  ...
  return (
    <ul className="space-y-1.5" data-testid="proxy-scan-findings">
      {detail.findings.map((finding) => (
        <li
          key={`${finding.cve_id}-${finding.package_name ?? ""}-${finding.package_version ?? ""}`}
          className="flex flex-wrap items-baseline gap-x-2 gap-y-0.5 text-xs"
        >
          <Badge
            variant="outline"
            className={`text-xs uppercase ${SEVERITY_BADGE[finding.severity] ?? ""}`}
          >
            {finding.severity}
          </Badge>
          <span className="font-medium">{finding.cve_id}</span>
...
export function ProxyScanPanel({
  ...
  return (
    <div className="space-y-3" data-testid="proxy-scan-panel">
      ...
      {/* Per-CVE detail, or the remedy when there is none. Without one of the
          two the panel reports a problem and offers no way to act on it. */}
      {view.kind === "verdict" && (
        <ProxyFindingDetail detail={proxyFindingDetail(view.entry)} />
      )}
    </div>
  );
}
