...
const STATE_BADGE: Record<string, string> = {
  ...
  pending_ingest: "text-muted-foreground bg-muted",
};
...
export function ProxyScanSummaryTiles({ summary }: { summary: ProxyScanSummary }) {
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
      ...
      />
      <SummaryTile
        testId="proxy-summary-total-digests"
        label="Distinct digests"
        count={summary.total_digests}
        hint={PROXY_SCAN_DISTINCT_DIGESTS_COPY}
      />
    </div>
  );
}
...
function pathColumns(
  ...
  return [
    ...
    },
    {
      id: "findings",
      header: "Findings",
      accessor: (e) => e.findings_count ?? -1,
      sortable: true,
      cell: (e) =>
        hasRecordedFindings(e) ? (
          <ProxySeverityBadges
