...
describe("ProxyScanSummarySection — listing", () => {
  ...
  it("points at where the per-CVE detail lives instead of implying there is none", () => {
    // The paged read deliberately omits per-CVE detail (one query per row
    // would make it an N+1). Repeating the per-artifact "no detail recorded"
    // copy here would assert something about the digests that this response
    // never looked up.
    stubQuery({ data: response() });
    ...
    expect(
      screen.getByText(/Open an artifact from the Artifacts tab/i),
    ).toBeInTheDocument();
    expect(
      screen.queryByText(/were not recorded for this digest/i),
    ).toBeNull();
  });

  it("reports distinct digests, reconciling the per-state counts", () => {
    stubQuery({ data: response() });

    render(<ProxyScanSummarySection repositoryKey="npm-remote" />);

    expect(screen.getByTestId("proxy-summary-total-digests")).toHaveTextContent(
      "7",
    );
  });
