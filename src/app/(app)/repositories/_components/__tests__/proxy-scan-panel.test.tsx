...
describe("ProxyScanPanel — verdicts", () => {
  ...
  it("renders not_scanned neutrally and never as clean", () => {
    stubQuery({
      data: {
        ...
        entry: entry({
          state: "not_scanned",
          not_scanned_reason: "scanning_disabled",
          scanned_at: null,
        }),
      },
    });
  ...
  it("lists the CVEs behind a vulnerable verdict when the backend records them", () => {
    // artifact-keeper#3395: the single-path read carries per-CVE detail for a
    // vulnerable entry. Showing only counts when the detail exists would leave
    // an operator whose build was just blocked with nothing to act on.
    stubQuery({
      data: {
        scan_on_proxy: true,
        proxy_scan_action: "fail_closed",
    ...
    expect(screen.getByTestId("proxy-scan-findings")).toBeInTheDocument();
    expect(screen.getByText("CVE-2021-44228")).toBeInTheDocument();
    expect(screen.getByText(/fixed in 2\.15\.0/i)).toBeInTheDocument();
    // The "no detail recorded" remedy is for the other case, not this one.
    expect(screen.queryByTestId("proxy-scan-no-cve-detail")).toBeNull();
  });

  it("names the remedy when a vulnerable verdict has no recorded detail", () => {
    // An empty recorded list is the real state for anything cached before
    // artifact-keeper#3395. It must never render as a findings table with no
    // rows, which would read as "we looked and found nothing".
    stubQuery({
      data: {
        scan_on_proxy: true,
        proxy_scan_action: "fail_closed",
    ...
    expect(screen.queryByTestId("proxy-scan-findings")).toBeNull();
    expect(
      screen.getByText(/Individual findings were not recorded for this digest/i),
    ).toBeInTheDocument();
    expect(
      screen.getByText(/ingest it into a hosted repository/i),
    ).toBeInTheDocument();
  });

  it("offers no per-CVE section for a state that has no findings to explain", () => {
    stubQuery({
      data: {
        scan_on_proxy: true,
        proxy_scan_action: "fail_closed",
