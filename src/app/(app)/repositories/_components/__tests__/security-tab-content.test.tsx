...
//
// `scanned` drives the artifact-scans query that #883 added: the green
...
function stubEmptyQueries({ scanned = false }: { scanned?: boolean } = {}) {
  mockUseQuery.mockImplementation((opts: { queryKey: unknown[] }) => {
    ...
    // Everything else — dt-status / dt-projects / dt-project-metrics /
    // dt-project-findings, and the package-analysis key the vendored and
    // install-script roll-ups share — resolves to no data, so neither the DT
    // section nor those roll-ups render.
...
describe("SecurityTabContent — CVE empty state (#3344)", () => {
  ...
  it("shows the green all-clear for an analyzable artifact that has been scanned", () => {
    stubEmptyQueries({ scanned: true });
  ...
  it("withholds the green all-clear from an analyzable artifact nothing has scanned (#883)", () => {
    // Both halves of the evidence are required. This half is #883's rule: an
    // unscanned artifact and a clean one must not share an icon and a colour.
    render(<SecurityTabContent artifact={art({ analyzable: true })} />);
    expect(screen.getByTestId("vulns-not-assessed")).toBeInTheDocument();
    expect(
      screen.queryByText(/No vulnerabilities detected for this artifact/i),
    ).toBeNull();
    expect(
      screen.getByText(/has not been scanned for vulnerabilities/i),
    ).toBeInTheDocument();
  });

  it("does not show a green all-clear for non-analyzable proxy-cached artifacts (#3344)", () => {
    // Even with an artifact-keyed scan on record, proxy content never earns
    // the all-clear here: the scan rows it would be drawn from cannot describe
    // proxy-cached bytes at all.
    stubEmptyQueries({ scanned: true });
